"""
Combined Recall Tables Generation Script

This script generates comprehensive LaTeX tables showing recall performance across 
different AI-generated image detection models, datasets, and prompting methods.
It processes evaluation results from both VLM models (Llama, Qwen) and the CoDE 
vision transformer to create unified comparison tables.

The script calculates recall metrics for each subset within datasets (e.g., different
AI generators like DALL-E, Stable Diffusion variants) and formats them into 
publication-ready LaTeX tables with proper formatting and statistical highlighting.

Features:
- Multi-model recall comparison (VLMs vs CoDE)
- Subset-level granular analysis for each dataset
- LaTeX table generation with bold highlighting for best performance
- Automatic subset name mapping and organization
- Standardized formatting for academic publications

Supported Models:
- VLMs: Llama 3.2 11B, Qwen2.5 7B
- Vision Transformer: CoDE (trained on D3)

Supported Datasets:
- D3: Contains real images and multiple AI-generated subsets
- DF40: DiffusionForensics dataset with various generators
- GenImage: Multi-generator synthetic image dataset

Processing Pipeline:
1. Load dataset mappings (image → generator subset)
2. Process rationale files to extract TP/FN counts per subset
3. Calculate recall scores for each model-method-dataset-subset combination
4. Generate LaTeX tables with proper formatting and highlighting
5. Save tables to output files

Usage:
    python results/combine_tables.py
    
Output Files:
    - LaTeX tables saved to RESULTS_OUTPUT_DIR
    - Individual tables for each dataset
    - Combined summary tables (if configured)

Configuration:
    Edit the configuration constants at the top of the script to adjust:
    - Model selection and naming
    - Dataset filtering
    - LaTeX formatting options
    - Subset name mappings

Note:
    This script processes the "rationales.jsonl" files generated by evaluation
    scripts, not the summary CSV files. It provides more detailed subset-level
    analysis than the macro F1-score summaries.
"""
